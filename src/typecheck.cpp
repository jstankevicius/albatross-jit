...
typecheck_exp(ExpNode *exp)
{
    ...
    case ExpNode::VarExp: {
        ...
        try {
            ...
            throw AlbatrossError("failed to read var_info for variable "
                                     + node->name,
                                 node->line_num,
                                 node->col_num,
                                 EXIT_FAILURE);
        ...
        if (n_args != n_params) {
            throw AlbatrossError(
                "Incorrect number of arguments supplied for function "
                    + node->name + ": expected " + std::to_string(n_params)
                    + ", got " + std::to_string(n_args),
                exp->line_num,
                exp->col_num,
                EXIT_TYPECHECK_FAILURE);
        }
...
typecheck_stmt(StmtNode *stmt, std::optional<Type> fun_ret_type = std::nullopt)
{
    ...
    case StmtNode::RetStmt: {
        ...
        Type ret_exp_type = node->ret_exp.has_value() ?
